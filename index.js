...
// Mutable references — filled during async init
let getContext = () => ({});
...
async function resolveImports() {
    // Try script.js first (standard SillyTavern path)
    try {
        const m = await import('../../../../script.js');
        if (typeof m.getContext === 'function')            getContext = m.getContext;
        ...
        console.warn('[iPhone Simulator] script.js import failed:', e);
    }

    // If extension_settings still null, try extensions.js
    if (!extension_settings) {
        try {
            const m2 = await import('../../../extensions.js');
            if (m2.extension_settings && typeof m2.extension_settings === 'object')
                extension_settings = m2.extension_settings;
        } catch(e) { /* ignore */ }
    }

    // Final fallback: plain object (settings will use localStorage)
