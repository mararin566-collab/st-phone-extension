...
// ============================================================
// Extension path when installed: /scripts/extensions/third-party/<name>/index.js
// So relative to this file:
//   ../../../../script.js              = /script.js  (main ST script)
//   ../../../../scripts/extensions.js  = /scripts/extensions.js

// Mutable references — filled by resolveImports()
let getContext = () => (typeof SillyTavern !== 'undefined' ? SillyTavern.getContext() : {});
...
async function resolveImports() {
    // Method 1: SillyTavern global object (recommended by official docs, works in all new versions)
    if (typeof SillyTavern !== 'undefined' && typeof SillyTavern.getContext === 'function') {
        getContext = SillyTavern.getContext.bind(SillyTavern);
        console.log('[iPhone Simulator] Using SillyTavern.getContext()');
    }

    // Method 2: Try importing extension_settings + saveSettingsDebounced from known paths
    // Path A: /scripts/extensions.js  (correct path per official docs)
    const pathsToTry = [
    ...
    for (const path of pathsToTry) {
        if (extension_settings) break;
        try {
            const m = await import(/* webpackIgnore: true */ path);
            if (m.extension_settings && typeof m.extension_settings === 'object') {
                extension_settings = m.extension_settings;
                console.log(`[iPhone Simulator] Got extension_settings from ${path}`);
            }
            if (typeof m.saveSettingsDebounced === 'function') {
                saveSettingsDebounced = m.saveSettingsDebounced;
            }
            if (typeof m.getContext === 'function' && !SillyTavern?.getContext) {
                getContext = m.getContext;
            }
        } catch(e) { /* try next */ }
    }

    // Method 3: Final fallback — use localStorage only
    if (!extension_settings) {
        extension_settings = {};
        console.warn('[iPhone Simulator] extension_settings not found, using localStorage fallback');
    }
}
...
// Returns true when extension_settings is not usable (use localStorage instead)
function useLS() { return !extension_settings || typeof extension_settings !== 'object'; }
